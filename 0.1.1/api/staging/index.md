...
[`stage`](https://yaugenst.github.io/advect/0.1.1/api/staging/#advect.stage) compiles one input signature into an immutable, optimized program that can be called repeatedly, differentiated, saved, and loaded. The [staging tutorial](https://yaugenst.github.io/advect/0.1.1/tutorials/staging/index.md) develops the complete workflow.
...
When examples are supplied without a target, [`stage`](https://yaugenst.github.io/advect/0.1.1/api/staging/#advect.stage) selects the newest revision every example provider can serve. With [`specs=`](https://yaugenst.github.io/advect/0.1.1/api/staging/#advect.stage) and no concrete provider, it defaults to `2024.12`. The selected target is stored in the graph, preserved by [`grad`](https://yaugenst.github.io/advect/0.1.1/api/transforms/#advect.grad) and [`vjp_program`](https://yaugenst.github.io/advect/0.1.1/api/staging/#advect.vjp_program), and enforced before runtime graph evaluation. Choosing an older target is the deliberate way to build a more portable artifact; Advect does not infer a minimum revision from the operations used by the function.
