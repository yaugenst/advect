...
The [operation-authoring guide](https://yaugenst.github.io/advect/0.1.1/development/adding-operations/index.md) explains the exact `InvocationCase`, `RawRuleCase`, `NumpySupportCase`, derivative-role, and lifetime evidence required for a new public form.
...
An outer `StagedProgram` envelope change instead runs Python static checks and `advect_core_tests`, with `test_stage_durability.py` as the focused regression owner. It does not require a wheel build when the enclosed runtime graph and PyO3 contract are unchanged. The [operation-authoring guide](https://yaugenst.github.io/advect/0.1.1/development/adding-operations/#staged-program-runtime-graph-and-native-changes) lists the proportional command sets.
...
The native snippet runner is necessary but not sufficient for browser support. When runnable snippets or the playground boundary changes, build the Pyodide wheel and run the browser lane described in the [documentation guide](https://yaugenst.github.io/advect/0.1.1/development/documentation/index.md).
...
- Array API operation changes run `uv run pytest packages/advect/tests/advect_grad_tests/test_array_api_operation_qualification.py` and `uv run python -m scripts.qualify_array_api_operations` for every affected revision, as shown in the [operation-authoring guide](https://yaugenst.github.io/advect/0.1.1/development/adding-operations/#array-api-forms-and-providers). Provider changes also run `uv run pytest packages/advect/tests/advect_array_api_compat_tests`. CuPy is qualified only on a CUDA host; record it as unverified when unavailable.
