...
A gradient is one special view of a derivative. For a general function, the derivative at a point is a linear map: a [JVP](https://yaugenst.github.io/advect/0.2.0/api/transforms/#advect.jvp) applies it to an input direction, while a [VJP](https://yaugenst.github.io/advect/0.2.0/api/transforms/#advect.vjp) applies its transpose to an output cotangent. Advect exposes both without first building a dense Jacobian.
...
[`vjp`](https://yaugenst.github.io/advect/0.2.0/api/transforms/#advect.vjp) evaluates the function and returns a one-shot [`Pullback`](https://yaugenst.github.io/advect/0.2.0/api/transforms/#advect.Pullback). Applying the pullback computes `J.T @ cotangent`:
...
[`linearize`](https://yaugenst.github.io/advect/0.2.0/api/transforms/#advect.linearize) captures the derivative once and returns a reusable [`LinearMap`](https://yaugenst.github.io/advect/0.2.0/api/transforms/#advect.LinearMap). It can apply the map or its transpose until it is closed:
...
The map belongs to one concrete call; it is reusable, but it is not a [`StagedProgram`](https://yaugenst.github.io/advect/0.2.0/api/staging/#advect.StagedProgram) or a cache across input points.

## Materialize the Jacobian only when useful

[`jacobian`](https://yaugenst.github.io/advect/0.2.0/api/transforms/#advect.jacobian) assembles the dense matrix and preserves array or pytree block shapes. The products above and the matrix are three views of the same local derivative:
