...
Dynamic transforms follow every call through Python. When the same computation will run repeatedly with matching inputs, [`stage`](https://yaugenst.github.io/advect/0.2.0/api/staging/#advect.stage) compiles that input signature, optimizes the graph once, and returns an immutable [`StagedProgram`](https://yaugenst.github.io/advect/0.2.0/api/staging/#advect.StagedProgram) for repeated execution.
...
Use [`ArraySpec`](https://yaugenst.github.io/advect/0.2.0/api/staging/#advect.ArraySpec) when no representative value is available. The [`kw_specs`](https://yaugenst.github.io/advect/0.2.0/api/staging/#advect.stage) argument declares keyword inputs, and [`StaticSpec`](https://yaugenst.github.io/advect/0.2.0/api/staging/#advect.StaticSpec) snapshots a compile-time Python value. Static values can control Python branches because they are known while staging:
...
Warm derivative calls execute their prebuilt graphs. They do not create a dynamic tape or run a reverse sweep. This is the reusable counterpart to the one-shot pullback returned by dynamic [`vjp`](https://yaugenst.github.io/advect/0.2.0/api/transforms/#advect.vjp).
...
The artifact contains the graph and its exact call contract, not Python code. Captured arrays and static values are snapshotted at compile time. A custom [primitive](https://yaugenst.github.io/advect/0.2.0/api/primitives/index.md) referenced by the graph must be imported or registered under the same stable name before loading, with an implementation that matches the saved program.

Provider-neutral functions written through `x.__array_namespace__()` can be staged against an explicit [Array API revision](https://yaugenst.github.io/advect/0.2.0/compatibility/array-api/index.md) and replayed by a compatible provider. NumPy-authored functions retain the separate [NumPy frontend](https://yaugenst.github.io/advect/0.2.0/api/numpy/index.md) contract. Save and load a program with the same Advect version.
