...
SciPy 1.18.1 compatibility functions are explicit `advect.scipy` entry points rather than transparent interception of direct `scipy.*` calls. The module is installed by the `advect[scipy]` extra, and an artifact that contains one of these functions needs `import advect.scipy` before `StagedProgram.from_dict()`.
