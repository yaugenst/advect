...
NumPy is Advect's first-class frontend, intercepted through NumPy's own protocols (`__array_ufunc__`, `__array_function__`, and constructor `like=` dispatch). The qualified range is NumPy 2.0-2.4. This build uses NumPy 2.4.6 and its Array API 2024.12 contract. Only the forms listed below are qualified.
