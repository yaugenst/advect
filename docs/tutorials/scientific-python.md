...
Ask an input for its
[Array API](../compatibility/array-api.md) namespace when the same function
should run on more than one provider:
...
body. For example, with
[`array-api-strict`](https://data-apis.org/array-api-strict/) installed:
...
`scipy.*` calls are not intercepted. The
[special-function reference](../api/scipy/special.md) and
[SciPy compatibility table](../compatibility/scipy.md) give the exact callable
...
The optional namespace also includes differentiable
[image filters](../api/scipy/ndimage.md) and
[solver callbacks](../api/scipy/solvers.md) used by
[implicit differentiation](implicit-differentiation.md).

## Preserve xarray labels

[`advect.xarray`](../api/xarray.md) registers
...
as dynamic pytrees. Data buffers are differentiable; dimensions, coordinates,
names, and attributes are static metadata restored on derivative results.
...
The [xarray boundary](../compatibility/xarray.md) is dynamic-only. Stage the raw
array kernel, call it with `field.data`, and restore labels outside the program
when reusable staging is needed.
