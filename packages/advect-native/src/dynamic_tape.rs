//! Concrete invocation-local autodiff tape backed by the shared SSA arena.
...
type LayoutSnapshot = (Vec<usize>, Option<(usize, usize)>);
...
impl DynamicTape {
    ...
    fn require_recording(&self) -> PyResult<()> {
        if self.consumed {
    ...
    fn prepare_layout(&self, plan: &LayoutPlan, literal_count: usize) -> PyResult<OperandLayout> {
        match plan {
            ...
            LayoutPlan::Mixed {
                ...
                let position_start = u32::try_from(self.operand_positions.len()).map_err(|_| {
                    PyValueError::new_err(
                        "dynamic tape operand-position arena exceeded its index range",
                    )
                })?;
                let literal_start = u32::try_from(self.literals.len()).map_err(|_| {
                    PyValueError::new_err("dynamic tape literal arena exceeded its index range")
                })?;
                let literal_count = u32::try_from(literal_count).map_err(|_| {
                    PyValueError::new_err("dynamic tape node has too many literals")
                })?;
    ...
    fn commit_layout(&mut self, plan: LayoutPlan, literals: Vec<Py<PyAny>>) {
    ...
    fn append_operation(
        ...
        let layout_plan = validate_operand_layout(parents.len(), parent_positions, literals.len())
        ...
        let layout = self.prepare_layout(&layout_plan, literals.len())?;
        ...
        self.commit_layout(layout_plan, literals);
...
fn take_node_literals(
    ...
    let Some((literal_start, literal_count)) = snapshot_layout(state, node_index, parent_count)?.1
    else {
        return Ok(());
    };
...
pub(super) fn snapshot_layout(
    ...
    match *state.operand_layouts.get(node_index).ok_or_else(|| {
...
pub(super) fn snapshot_operands(
    ...
    let parents = state
        ...
        .ok_or_else(|| PyRuntimeError::new_err("dynamic tape edge range is invalid"))?
        .to_vec();
    let (parent_positions, literal_range) = snapshot_layout(state, node_index, parents.len())?;
